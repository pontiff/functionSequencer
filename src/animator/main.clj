(load-file "/home/user/code/animator/src/animator/utl.clj")

(ns main
  (:require [net.cgrand.enlive-html :as html] )
  (:use seesaw.core seesaw.mig seesaw.dev clojure.java.io clojure.math.numeric-tower animator.utl))
 ...
 (defn writeJS []
     (map->timeline TIMELINE-MAP STATEMAP "/home/user/code/svg-lab/pub/js/mainVideo/timeline.js" "elements.mainVideo")
     (map->statefile STATEMAP  "/home/user/code/svg-lab/pub/js/mainVideo/stateMap.js"  "elements.mainVideo"))

     
(comment

  (load-file "src/animator/main.clj")
  (writeJS)
...
(for [[k v] @STATEMAP :when (= 0 (v :keyFrame))] k)
  ...
  (listen fcb1 :action
      (fn [e]
            (if (not (number? (selection fclb1)))
              ...
              ;  existingIdx (first (for [[k v] @STATEMAP :when (= (Integer. (selection fclb2))(v :keyFrame))] k)) ]
...
;;;;;;ELEMENTS->FRAMES->CODE PANEL
 ...
 (-> efc-ff pack! show!)
 (config! efclb1 :model (list* "null" ELEMENTS ))
...
(listen efcb1 :action
         (fn [ e ]
           (let [frames (selection efclb2 {:multi? true}) ]
             (doseq [v frames]
               (removeFrame TIMELINE-MAP TIMELINE-FILE (Integer. v)))
